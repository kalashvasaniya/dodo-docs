...
Security is critical when handling webhooks. Always verify webhook signatures and use HTTPS endpoints.

### Signature Verification

Every webhook request includes a `webhook-signature` header—an HMAC SHA256 signature that proves the webhook came from Dodo Payments.
...
<Step title="Construct Signed Content">
  Concatenate the `webhook-id`, `webhook-timestamp`, and stringified payload with periods (`.`):
  
  ```
  {webhook-id}.{webhook-timestamp}.{stringified-payload}
  ```
</Step>

<Step title="Compute HMAC">
  Calculate the HMAC SHA256 signature using your webhook secret key from the Dodo Payments dashboard.
</Step>

<Step title="Compare Signatures">
  Compare your computed signature with the received `webhook-signature` header. If they match, the webhook is authentic.
</Step>
</Steps>

<Info>
We follow the [Standard Webhooks](https://standardwebhooks.com/) specification. Use the official [Standard Webhooks libraries](https://github.com/standard-webhooks/standard-webhooks/tree/main/libraries) for automatic signature verification in your preferred language.
</Info>

<Warning>
**Never skip signature verification in production.** Without verification, malicious actors could send fake webhooks to your endpoint.
</Warning>
...
Your webhook endpoint must return the appropriate HTTP status code:

<ResponseField name="2xx" type="Success">
Webhook processed successfully. Any 2xx status code acknowledges receipt.
</ResponseField>

<ResponseField name="4xx/5xx" type="Failure">
Any non-2xx response is treated as a delivery failure and triggers the retry mechanism.
</ResponseField>
