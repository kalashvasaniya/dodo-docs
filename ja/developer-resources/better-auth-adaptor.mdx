...
<Info>
  Webhook プラグインは、Dodo Payments からのリアルタイムの支払いイベントを
  セキュアな署名検証で処理します。デフォルトのエンドポイントは
  <code>/api/auth/dodopayments/webhooks</code> です。
</Info>

<Steps>
<Step title="Webhook シークレットの生成と設定">
Dodo Payments ダッシュボードでエンドポイント URL（例：<code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）のために webhook シークレットを生成し、<code>.env</code> ファイルに設定します：
...
<AccordionGroup>
  <Accordion title="プラグインオプション">
    - <b>client</b>（必須）：DodoPayments クライアントインスタンス - <b>createCustomerOnSignUp</b>（オプション）：ユーザーサインアップ時に顧客を自動作成 - <b>use</b>（必須）：有効にするプラグインの配列（チェックアウト、ポータル、Webhook）
  </Accordion>
