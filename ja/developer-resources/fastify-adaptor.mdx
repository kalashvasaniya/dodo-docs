...
<Tab title="カスタマーポータルハンドラー">
  <Info>
  このハンドラーを使用して、顧客が Dodo Payments のカスタマーポータルを介してサブスクリプションや詳細を管理できるようにします。
  </Info>
  <CodeGroup>
...
<Tab title="ウェブフックハンドラー">
  <Info>
  このハンドラーを使用して、Fastify アプリで Dodo Payments のウェブフックイベントを安全に受信して処理します。
  </Info>
  <CodeGroup>
...
<Warning>
<code>productId</code> が欠けている場合、ハンドラーは 400 レスポンスを返します。無効なクエリパラメータも 400 レスポンスになります。
</Warning>
...
カスタマーポータルルートハンドラーを使用すると、Dodo Payments のカスタマーポータルを Fastify アプリケーションにシームレスに統合できます。
...
```
あなたは専門の Fastify 開発者アシスタントです。あなたの仕事は、ユーザーが既存の Fastify プロジェクトに @dodopayments/fastify アダプタを統合する手順を案内することです。

@dodopayments/fastify アダプタは、Dodo Payments のチェックアウト、カスタマーポータル、およびウェブフック機能のルートハンドラーを提供し、Fastify アプリに直接接続できるように設計されています。
...
- ウェブフックルートハンドラー (Dodo Payments のウェブフックイベントを受信するため)
- すべて (3 つすべてを統合)"
...
**統合**:
Fastify アプリに 2 つのルートを作成します — 1 つは静的 (GET) 用、もう 1 つは動的 (POST) 用のチェックアウトです。
...
POST (動的チェックアウト) は、支払い詳細 (一回限りまたはサブスクリプション) を含む JSON ボディを期待します。戻り値: {"checkout_url": "https://checkout.dodopayments.com/..."}。完全な POST スキーマについては、次を参照してください:
...
セキュリティノート: シークレットをバージョン管理にコミットしないでください。ローカルでは .env ファイルを使用し、デプロイ環境 (例: AWS、Vercel、Heroku など) ではシークレットマネージャーを使用してください。